...
decaf.extend(EC2Client.prototype, {
    ...
    createSecurityGroup: function(config) {
        var groupResult = this.client.createSecurityGroup(new com.amazonaws.services.ec2.model.CreateSecurityGroupRequest().withGroupName(config.name).withDescription(config.description)),
        ...
        var permissions = [];
        decaf.each(config.inbound, function (inbound) {
        ...
        permissions = [];
        decaf.each(config.outbount, function (outbount) {
            var permission = new com.amazonaws.services.ec2.model.IpPermission()
                .withIpProtocol(outbount.protocol || -1)
                .withFromPort(outbount.fromPort || -1)
                .withToPort(outbount.toPort || -1)
                .withIpRanges(outbount.ipRange)
            permissions.push(permission);
        });
