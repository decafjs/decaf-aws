...
function permissionsInfo(permission) {
    ...
    return {
        protocol : protocol,
        fromPort : 0 + permission.getFromPort(),
        toPort   : 0 + permission.getToPort(),
        ipRange  : ranges
    };
}
...
decaf.extend(EC2Client.prototype, {
    ...
    createSecurityGroup: function(name, options) {
        var groupResult = this.client.createSecurityGroup(new com.amazonaws.services.ec2.model.CreateSecurityGroupRequest().withGroupName(name).withDescription(options.description)),
            groupId     = String(groupResult.getGroupId()),
            ingress     = new com.amazonaws.services.ec2.model.AuthorizeSecurityGroupIngressRequest(),
            egress      = com.amazonaws.services.ec2.model.AuthorizeSecurityGroupEgressRequest();

        var permissions = [];
        decaf.each(options.inbound || [], function (inbound) {
            var permission = new com.amazonaws.services.ec2.model.IpPermission()
                .withIpProtocol(inbound.protocol || -1)
                .withFromPort(inbound.fromPort || -1)
                .withToPort(inbound.toPort || -1)
                .withIpRanges(inbound.ipRange || [])
            permissions.push(permission);
        });
        this.client.authorizeSecurityGroupIngress(ingress.withIpPermissions(permissions).withGroupId(groupId));

        permissions = [];
        decaf.each(options.outbound || [], function (outbound) {
            var permission = new com.amazonaws.services.ec2.model.IpPermission()
                .withIpProtocol(outbound.protocol || -1)
                .withFromPort(outbound.fromPort || -1)
                .withToPort(outbound.toPort || -1)
                .withIpRanges(outbound.ipRange || [])
            permissions.push(permission);
        });
        this.client.authorizeSecurityGroupEgress(egress.withIpPermissions(permissions).withGroupId(groupId));
        return groupId;
    },
    removeSecurityGroup: function(groupId) {
        this.client.deleteSecurityGroup(new com.amazonaws.services.ec2.model.DeleteSecurityGroupRequest().withGroupId(groupId));
    },
    // Instances
